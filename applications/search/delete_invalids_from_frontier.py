...
from datamodel.search.MmaranoBhtruon1_datamodel import MmaranoBhtruon1Link
...
@Deleter(MmaranoBhtruon1Link)
@Getter(MmaranoBhtruon1Link)
class DeleteFrontierFrame(IApplication):
    ...
    def update(self):
        ...
        ls = self.frame.get(MmaranoBhtruon1Link)
        print "Found ", len(ls), " links."
        count = 0
        for l in ls:
            if not is_valid(l.full_url):
                self.frame.delete(MmaranoBhtruon1Link, l)
                count += 1
...
class Simulation(object):
    ...
    def __init__(self, address, port):
        ...
        objectless_connector = ObjectlessSpacetimeConnection(
            "ResetFrontier_MmaranoBhtruon1".format(CrawlerFrame.app_id),
